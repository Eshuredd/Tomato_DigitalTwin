...
This document tracks the staged migration from the legacy Streamlit UI to a Next.js frontend. The current phase adds candidate action simulation and deterministic recommendation after accepted canonical twin state while keeping FastAPI as the authoritative backend.
...
One-day advancement requires an active session, accepted disease evidence, accepted canonical water lineage, accepted canonical twin, reviewed weather for the derived next date, valid recent-irrigation input, and no conflicting pending request. The browser derives the target date from `twin.current_state.observed_at` plus one calendar day and does not expose arbitrary target-date selection. Weather review can clear the transient displayed water response without clearing `latestWaterObservationId`, `latestWaterSequence`, or the canonical twin; advancement therefore relies on those canonical lineage fields and the twin timestamp rather than reconstructing local water state. Fetched weather must match both the required date and unchanged fetched values; edited same-date weather and fully manual weather require explicit acknowledgement, while fetched weather for another date remains blocked until weather is retrieved for the required date. Advancement requests use stable payload signatures and reuse the same `advancement_id` for unchanged user-triggered retries. The browser validates the `AdvanceOneDayResponse`, including nested water and twin `state_id` consistency, displays created versus reused status, and applies retry transitions conservatively: new advancements replace canonical water and twin, catch-up retries replace water then refresh the authoritative twin, current retries preserve newer local twin state, historical retries remain technical-only, and malformed sequence metadata is never accepted as valid canonical state.

Candidate action simulation requires an active session and accepted canonical twin. The UI selects all four supported actions by default for each new active session or canonical twin source that has no valid accepted simulation, and allows a user-selected non-empty subset: `IRRIGATE_NOW`, `IRRIGATE_IN_6H`, `IRRIGATE_TOMORROW_AM`, and `NO_IRRIGATION_24H`. If retained simulation metadata proves exact current state, canonical twin, source signature, and accepted action membership, component remounts and equivalent canonical refreshes restore that accepted subset instead of reverting to all four actions. Requests go directly through the typed endpoint wrapper to `POST /sessions/{state_id}/simulate-actions` with `{ state_id, actions }`. Changing the logical selected-action set after a simulation is accepted clears the accepted simulation, recommendation, and accepted source metadata without automatically submitting. The response validator accepts backend numeric projections as finite numbers without clamping or non-negativity rewrites, validates timezone-aware `simulated_at`, and rejects malformed action or stress enum values. Request-specific validation requires exact state ID and an exact one-result-per-requested-action set. Backend response order is allowed to vary; rendering normalizes to the submitted workflow order without recomputing values.

Deterministic recommendation requires an accepted simulation for the current canonical twin source signature. The UI never submits automatically and calls `POST /sessions/{state_id}/recommend` with no body. Accepted simulation actions and source signatures are stored separately from active request metadata. Before enabling recommendation, the browser recomputes the current canonical twin and simulation source, requires an exact match to the accepted metadata, requires the accepted simulation result actions to match the accepted action list, and blocks stale or missing source proofs. The response validator requires a valid action, irrigation constraint, caution reasons, string decision reason codes, structured evidence object, boolean inspection advisory, and timezone-aware `recommended_at`. The browser stores a recommendation only when its `state_id` matches the active state, source signatures are unchanged, and the backend-chosen action appears in the accepted simulation. It does not rank simulation results, select the chosen action, generate narration, or provide pesticide, fertiliser, or treatment instructions.

Equivalent canonical twin refreshes preserve valid decisions by comparing a stable canonical decision signature rather than object identity or `snapshot_created`. Changed snapshots, changed canonical timestamps, changed disease or water fields in the canonical twin, missing canonical twins, and catch-up advancement refresh failures clear stale simulation and recommendation outputs. Narration, history, actual-action recording, farms, plots, crop cycles, and Streamlit removal remain unimplemented.

Browser-level Playwright coverage is not part of this phase. The current frontend test boundary is Vitest, jsdom, React Testing Library, API-client unit coverage, reducer tests, and pure conversion/signature tests.
