...
Candidate action simulation requires an active session and accepted canonical twin. The UI selects all four supported actions by default and allows a user-selected non-empty subset: `IRRIGATE_NOW`, `IRRIGATE_IN_6H`, `IRRIGATE_TOMORROW_AM`, and `NO_IRRIGATION_24H`. Requests go directly through the typed endpoint wrapper to `POST /sessions/{state_id}/simulate-actions` with `{ state_id, actions }`. The response validator accepts backend numeric projections as finite numbers without clamping or non-negativity rewrites, validates timezone-aware `simulated_at`, and rejects malformed action or stress enum values. Request-specific validation requires exact state ID and an exact one-result-per-requested-action set. Backend response order is allowed to vary; rendering normalizes to the submitted workflow order without recomputing values.

Deterministic recommendation requires an accepted simulation for the current canonical twin. The UI never submits automatically and calls `POST /sessions/{state_id}/recommend` with no body. The response validator requires a valid action, irrigation constraint, caution reasons, string decision reason codes, structured evidence object, boolean inspection advisory, and timezone-aware `recommended_at`. The browser stores a recommendation only when its `state_id` matches the active state and the backend-chosen action appears in the accepted simulation. It does not rank simulation results, select the chosen action, generate narration, or provide pesticide, fertiliser, or treatment instructions.
