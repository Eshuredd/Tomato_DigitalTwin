...
const defaults = (targetDate?: string): WeatherReviewInput => ({ target_date: targetDate ?? localCalendarDate(), tmin_c: '', tmax_c: '', humidity_pct: '', wind_speed_mps: '', shortwave_radiation_sum_mj_m2: '', rainfall_mm: '', eto_reference_feed: '' });
const fields = [
...
export function WeatherWorkflow({ stateId, lockedTargetDate, onAcceptedChange }: { stateId: string; lockedTargetDate?: string; onAcceptedChange?: (value: ReviewedWeather | undefined) => void }) {
  const form = useForm<WeatherReviewInput>({ defaultValues: defaults(lockedTargetDate) }); const targetDate = useWatch({ control: form.control, name: 'target_date' });
  ...
  async function retrieve() { const parsed = calendarDateSchema.safeParse(targetDate); if (!parsed.success) { form.setError('target_date', { message: parsed.error.issues[0]?.message }); return; } const result = await snapshot.refetch(); if (result.data) { form.reset(weatherValues(result.data)); setProvenance('fetched_reviewed'); setReviewed(undefined); onAcceptedChange?.(undefined); } }
  function edit(name: keyof WeatherReviewInput, value: string, onChange: (value: string) => void) { onChange(value); setProvenance('manual'); setReviewed(undefined); onAcceptedChange?.(undefined); form.clearErrors('root'); }
  function accept(values: WeatherReviewInput) { const parsed = weatherReviewSchema.safeParse(values); if (!parsed.success) { for (const issue of parsed.error.issues) { const name = issue.path[0] as keyof WeatherReviewInput; if (name) form.setError(name, { message: issue.message }); } return; } const next = reviewWeather(stateId, { ...values, ...(lockedTargetDate ? { target_date: lockedTargetDate } : {}) }, provenance); setReviewed(next); onAcceptedChange?.(next); }
  ...
  return <SectionCard title="Weather review"><Text>FastAPI retrieves Open-Meteo weather using this session’s authoritative coordinates. Review every value before accepting it as an unsaved input.</Text>
    <Controller control={form.control} name="target_date" render={({ field, fieldState }) => <FormField label="Target date (YYYY-MM-DD)" value={lockedTargetDate ?? field.value} editable={!lockedTargetDate} onBlur={field.onBlur} onChangeText={(value) => edit('target_date', value, field.onChange)} error={fieldState.error?.message} keyboardType="numbers-and-punctuation" />} />
