...
import { localDateTimeInput } from '@/lib/dates/local-date';
import { reviewIrrigation, type IrrigationDraft, type IrrigationMode, type ReviewedIrrigation } from './drafts';

const defaults = (): IrrigationDraft => ({ mode: 'none', timestamp: localDateTimeInput(), directDepth: '', totalLitres: '', litresArea: '', emitterCount: '', emitterFlow: '', runtimeMinutes: '', dripArea: '' });
export function IrrigationWorkflow({ stateId }: { stateId: string }) {
  const form = useForm<IrrigationDraft>({ defaultValues: defaults() }); const mode = useWatch({ control: form.control, name: 'mode' }); const [reviewed, setReviewed] = useState<ReviewedIrrigation>(); const [error, setError] = useState<string>();
  function change(name: keyof IrrigationDraft, value: string) { form.setValue(name, value as never); setReviewed(undefined); setError(undefined); }
  function accept(values: IrrigationDraft) { try { setReviewed(reviewIrrigation(stateId, values)); setError(undefined); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Irrigation input is invalid.'); } }
