...
export function useSession(stateId: string) { return useQuery<SessionSummary>({ queryKey: queryKeys.session(stateId), queryFn: ({ signal }) => getSession(stateId, signal), enabled: Boolean(stateId), retry: false, staleTime: 30_000 }); }
