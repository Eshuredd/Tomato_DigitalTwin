...
export function WorkflowWorkspace({ stateId }: { stateId: string }) {
  ...
  const [identities] = useState(() => new WorkflowIdentityStore()); const [activeStage, setActiveStage] = useState<StageId>("disease"); const [mutationPending, setMutationPending] = useState(false);
  ...
  const [water, setWater] = useState<AcceptedWaterResult>(); const [twin, setTwin] = useState<AcceptedTwinResult>(); const [, setAdvancement] = useState<AcceptedAdvancementResult>();
  ...
  const waterSource = disease && acceptedWeather && acceptedIrrigation ? canonicalJson({ state_id: stateId, disease_generation: disease.fileSignature, disease: disease.response, weather: acceptedWeather.signature, irrigation: acceptedIrrigation.semanticSignature }) : undefined;
  const waterCurrent = Boolean(water && waterSource && weatherCurrent && irrigationCurrent && water.sourceSignature === waterSource);
  const twinSource = disease && waterCurrent && water ? twinSourceSignature(stateId, disease.response, water.response) : undefined; const twinCurrent = Boolean(twin && twinSource && twin.sourceSignature === twinSource);

  const steps = useMemo<WorkflowStep[]>(() => stageIds.map((id, index) => {
    const completed = id === "session" ? recognized : id === "disease" ? Boolean(disease) : id === "weather" ? weatherCurrent : id === "irrigation" ? irrigationCurrent : id === "water" ? waterCurrent : id === "twin" ? twinCurrent : false;
    const available = id === "disease" ? recognized : id === "weather" ? Boolean(disease) : id === "irrigation" ? weatherCurrent : id === "water" ? Boolean(disease && weatherCurrent && irrigationCurrent) : id === "twin" ? Boolean(disease && waterCurrent) : false;
    const future = index >= 6; const interactive = completed || available;
    return { id, label: workflowStepLabels[index], status: activeStage === id && interactive ? "active" : completed ? "completed" : future || !available ? "blocked" : "available", prerequisite: future ? "Milestone 5 or later" : id === "water" ? "current disease, weather and irrigation" : id === "twin" ? "current disease and water lineage" : id === "disease" ? "recognized state ID" : id === "weather" ? "accepted disease evidence" : id === "irrigation" ? "accepted reviewed weather" : undefined, interactive };
  }), [activeStage, disease, irrigationCurrent, recognized, twinCurrent, waterCurrent, weatherCurrent]);
  ...
  function invalidateDownstream() { setAdvancement(undefined); identities.clearWater(); identities.clearAdvancement(); }
  ...
  return <><PageIntro eyebrow="Guided workflow" title="Deterministic water and canonical state" description="Build the current FastAPI water lineage, update the canonical twin, then explicitly advance exactly one day when ready." />
    <Card className="mb-5"><CardHeader><CardTitle>State-scoped progress</CardTitle><CardDescription>The first six stages are real workflow operations. Simulation and later decisions remain blocked for Milestone 5.</CardDescription></CardHeader><CardContent><WorkflowStepper steps={steps} onStepSelect={(step) => setActiveStage(step.id as StageId)} /></CardContent></Card>
