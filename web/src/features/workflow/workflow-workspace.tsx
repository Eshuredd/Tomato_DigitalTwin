...
export function WorkflowWorkspace({ stateId }: { stateId: string }) {
  ...
  return <><PageIntro eyebrow="Guided workflow" title="Deterministic water and canonical state" description="Build the current FastAPI water lineage, update the canonical twin, then explicitly advance exactly one day when ready." />
    <Card className="mb-5"><CardHeader><CardTitle>State-scoped progress</CardTitle><CardDescription>The first six stages are real workflow operations. Simulation and later decisions remain blocked for Milestone 5.</CardDescription></CardHeader><CardContent><WorkflowStepper steps={steps} onStepSelect={(step) => { coordinator.cancel(); setActiveStage(step.id as StageId); }} /></CardContent></Card>
    <div className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_19rem]"><main className="min-w-0">
      ...
      {activeStage === "twin" ? disease && water && waterCurrent && waterSource ? <TwinStage stateId={stateId} disease={disease} water={water} current={twinCurrent ? twin : undefined} advancement={advancement} identityStore={identities} workflowSourceSignature={waterSource} coordinator={coordinator} onAccepted={setTwin} onAdvancementRecovered={(value) => setAdvancement(value)} onAdvanced={({ water: nextWater, twin: nextTwin, advancement: nextAdvancement }) => { setWater(nextWater); if (nextTwin) setTwin(nextTwin); setAdvancement(nextAdvancement); }} /> : <Blocked title="Twin update is blocked" description="Current disease evidence and canonical water lineage are required." /> : null}
