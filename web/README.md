...
Candidate action simulation requires an active session and an accepted canonical twin snapshot. All four supported candidate actions are selected by default for a new active session or canonical twin source: `IRRIGATE_NOW`, `IRRIGATE_IN_6H`, `IRRIGATE_TOMORROW_AM`, and `NO_IRRIGATION_24H`. The user may submit any non-empty subset explicitly through `POST /sessions/{state_id}/simulate-actions`; the browser sends only `{ "state_id": stateId, "actions": [...] }`. Changing the logical selected-action set after accepting a simulation clears the old simulation, recommendation, and their accepted source metadata without submitting automatically.
